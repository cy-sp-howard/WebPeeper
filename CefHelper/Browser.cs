...
namespace CefHelper
{
    static public class Browser
    {
        ...
        static public ChromiumWebBrowser WebBrowser { get; private set; }
        static public void CefSettingInit(string defaultUserAgent, string localesPath, string settingPath, string subprocessPath, bool clearUserData)
        {
            ...
            settings.RegisterScheme(new CefCustomScheme()
            {
                ...
                SchemeHandlerFactory = new CefSchemeHandlerFactory() { BlishHudSchemeRequested = OnBlishHudSchemeRequested },
            });
        ...
        static public Task<bool> Create(string defaultUrl, int frameRate)
        {
            ...
            if (WebBrowser == null || WebBrowser.IsDisposed)
            {
                ...
                WebBrowser = new ChromiumWebBrowser(defaultUrl, browserSetting);
                ...
                WebBrowser.LoadError += OnUrlLoadError;
                ...
                {
                    MainFrameChanged = OnMainFrameChanged
                };
                ...
                {
                    FocusNodeChanged = OnFocusedNodeChanged,
        ...
        #region OnEvents;
        static (Stream, string) OnBlishHudSchemeRequested(IRequest request)
        {
        ...
        static void OnContextCreated(IFrame frame)
        {
        ...
        static void OnFocusedNodeChanged(IDomNode node)
        {
        ...
        static void OnMainFrameChanged()
        {
            FocusedChanged?.Invoke(false);
        }
        static void OnTitleChanged(object sender, TitleChangedEventArgs e)
        {
            TitleChanged?.Invoke(e.Title);
        }
        static void OnFrameLoadStart(object sender, FrameLoadStartEventArgs e)
        {
            FrameLoadStart?.Invoke();
        }
        static void OnUrlLoadError(object sender, LoadErrorEventArgs e)
        {
            UrlLoadError?.Invoke(e.FailedUrl);
        }
        static void OnFullscreenModeChanged(bool isFullscreen)
        {
            FullscreenModeChanged?.Invoke(isFullscreen);
        }
        static void OnLoadingStateChanged(object sender, LoadingStateChangedEventArgs e)
        {
            LoadingStateChanged?.Invoke(e.CanGoBack, e.CanGoForward, e.IsLoading);
        }
        static void OnAddressChanged(object sender, AddressChangedEventArgs e)
        {
            AddressChanged?.Invoke(e.Address);
        }
        static void OnPaint(object sender, OnPaintEventArgs e)
        {
